...
from lightning_cil.methods.ewc import EWC
...
class CLI(cli.LightningCLI):
    ...
    def run(self):
        ...
        for task_id in range(num_tasks):
            ...
            # Estimate Fisher and consolidate after each task
            model.consolidate(datamodule=dm, device=trainer.strategy.root_device)
            model.snapshot_prev_model()

        trainer.test(model, dm)

if __name__ == "__main__":
    CLI(EWC, CIFAR100DataModule, save_config_callback=None, run=True)
