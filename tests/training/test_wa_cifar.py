...
from lycil.learner.wa import WA
...
def test_wa_cifar100(accelerator: str, is_dummy_training: bool):
    ...
    dm = HFDataModule(
        ...
        train_loader_kwargs={"batch_size": BATCH_SIZE, "shuffle": True, "num_workers": 8},
    ...
    for task_idx, _ in enumerate(N_CLASS_PER_TASK):
        ...
        trainer = L.Trainer(
            ...
            precision="16-mixed",
            logger=WandbLogger(
                name=f"wa_cifar100_T{len(N_CLASS_PER_TASK)}_task{task_idx}",
                project="lycil",
                log_model=False,
                tags=["wa", "cifar100"],
                group=_EXP_NAME,
...
if __name__ == "__main__":
    test_wa_cifar100(accelerator="cuda", is_dummy_training=False)
