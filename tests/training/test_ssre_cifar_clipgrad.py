...
from lycil.learner.ssre import SSRE
...
def test_ssre_cifar100(device: str, is_dummy_training: bool):
    ...
    for task_idx, _ in enumerate(N_CLASS_PER_TASK):
        ...
        trainer = L.Trainer(
            ...
            precision="16-mixed",
            logger=WandbLogger(
                name=f"clipgrad_ssre_cifar100_T{len(N_CLASS_PER_TASK)}_task{task_idx}",
                project="lycil",
                log_model=False,
                tags=["ssre", "cifar100"],
                group=_EXP_NAME,
            ...
            gradient_clip_val=1.0 if task_idx == 0 else 0.5,
        )
...
if __name__ == "__main__":
    test_ssre_cifar100(device="cuda", is_dummy_training=False)
