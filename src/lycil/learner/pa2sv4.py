...
class PASS(BaseLearner):
    ...
    def __init__(
        ...
        self._prototype_radii: list[torch.Tensor] = []   # each is [D]
    ...
    def prototype_loss(self, batch_size: int) -> torch.Tensor:
        """
        Prototype replay with class-specific, per-dimension radii.
        """
        ...
        ).to(self.device)  # [C_old, D]

        radius_bank = torch.stack(
            self._prototype_radii[: self.num_old_classes], dim=0
        ).to(self.device, dtype=proto_bank.dtype)  # [C_old, D]
        ...
        sampled_radii = radius_bank[cls_idx]      # [B, D]
    ...
    def update_prototypes(self, dm) -> None:
        ...
        with torch.no_grad():
            ...
            for class_idx in range(self.num_old_classes, self.num_seen_classes):
                ...
                # per-dimension radius: [D]
                if class_feats.size(0) > 1:
                    class_radius = class_feats.var(dim=0, unbiased=True).sqrt()
                else:
                    class_radius = torch.zeros_like(class_proto)

                new_radii.append(class_radius.detach().cpu())

            self._prototypes.extend(new_prototypes)
            self._prototype_radii.extend(new_radii)

            if self._prototype_radii:
                all_radii = torch.stack(self._prototype_radii, dim=0)  # [C, D]
                self._mean_radius = float(all_radii.mean().item())
            else:
